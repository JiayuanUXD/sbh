...
5. 未被有效举报暂停。
6. ~~媒体完整且可读，MVP 至少 3 张有效图片。~~ **已于 2026-08-19 废除**：
   媒体数量不再参与前台可见性，无图房源照常曝光并走缺省图降级。
   「至少 3 张」只剩**提交审核**一道门（`listing-completeness.ts` 的
...
11. 租赁还需可租、可用日期未结束。
12. ~~`last_effective_maintained_at` 不命中陈旧排除规则。~~ **已于 2026-08-26 废除**：
    用户确认陈旧只触发 `listing-stale-maintenance` 维护待办，不影响公开可见性。
    `updatedAt` / `last_effective_maintained_at` 距今多久都不能单独把房源移出公开供给；
    若同时命中草稿、未审核、冻结等其他失效条件，仍按对应条件排除。条号保留不重排，
    以便和既有设计、代码注释及历史验收记录对照。
...
至少覆盖草稿、未审核、冻结、举报、媒体不足、位置停用、商户停用/过期、
服务城市不覆盖、关系重叠、已出租和逻辑删除，并断言所有消费者解析出的
Listing ID 集合一致。另需覆盖陈旧边界：仅陈旧时仍然可见；陈旧与其他
失效条件叠加时，按其他失效条件排除。
