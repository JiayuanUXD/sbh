...
describe('POST /api/mini/v1/inquiries', () => {
  ...
  it('Mini 显式禁用 visitorRef，即使共享服务返回该字段也保持精确 DTO 与日志边界', async () => {
    const visitorRef = '0123456789abcdef0123456789abcdef'
    io.submitPublicInquiry.mockResolvedValue({ idempotent: false, targetResolution: 'listing', visitorRef })
    const result = await POST(routeRequest())

    expect(result.status).toBe(200)
    expect(await json(result)).toEqual({
      ok: true,
      data: { accepted: true, acceptedExisting: false, targetResolution: 'listing' },
      meta: { requestId: result.headers.get('x-request-id') },
    })
    expect(io.submitPublicInquiry.mock.calls[0]?.[0].trustedVisitorRef).toBeNull()
    expect(JSON.stringify([io.loggerInfo.mock.calls, io.loggerWarn.mock.calls, io.loggerError.mock.calls]))
      .not.toContain(visitorRef)
  })
