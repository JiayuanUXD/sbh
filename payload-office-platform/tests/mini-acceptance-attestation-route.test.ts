...
vi.mock('payload', async (importOriginal) => ({ ...(await importOriginal<typeof import('payload')>()), getPayload: io.getPayload }))
...
const runtimeConfig = { deploymentGitCommitSha: 'a'.repeat(40), deploymentRevision: 'rev-1', attestationSecret: key, operatorBootstrapSecret: Uint8Array.from(Array.from({ length: 32 }, (_, i) => i + 33)), dbFingerprintAllowlist: [databaseFingerprint(identity, key)] }

beforeEach(() => { io.getPayload.mockReset(); io.readConfig.mockReset().mockReturnValue(runtimeConfig) })

describe('GET acceptance attestation', () => {
  it('认证失败时同形 404 且不初始化 Payload', async () => {
    const response = await GET(new Request('https://example.test/api/mini/v1/acceptance/attestation', { headers: { 'x-sbh-acceptance-bootstrap': Buffer.from(Array.from({ length: 32 }, (_, i) => i + 34)).toString('base64url') } }))
    expect(response.status).toBe(404)
    expect(io.getPayload).not.toHaveBeenCalled()
  })

  it('超长 bootstrap header 在认证前拒绝且不初始化 Payload', async () => {
    const response = await GET(new Request('https://example.test/api/mini/v1/acceptance/attestation', { headers: { 'x-sbh-acceptance-bootstrap': 'x'.repeat(129) } }))
    expect(response.status).toBe(404)
    expect(io.getPayload).not.toHaveBeenCalled()
  })
  ...
  it('disabled 时仍在认证前拒绝且不初始化 Payload', async () => {
    ...
    const response = await GET(new Request('https://example.test/api/mini/v1/acceptance/attestation', { headers: { 'x-sbh-acceptance-bootstrap': operatorSecret } }))
    expect(response.status).toBe(404)
    expect(io.getPayload).not.toHaveBeenCalled()
  })
  ...
  it('认证通过后才执行只读 probe 并返回 opaque fingerprint', async () => {
    ...
    expect(query).toHaveBeenCalledWith({ text: 'SELECT current_database() AS "databaseName", inet_server_addr()::text AS "serverAddress", inet_server_port() AS "serverPort"', values: [] })
