...
describe('共享公开询盘领域服务', () => {
  const visitorRef = '0123456789abcdef0123456789abcdef'
  const storedVisitorRef = 'fedcba9876543210fedcba9876543210'

  it('首次创建只保存并返回 adapter 解析的可信 visitorRef', async () => {
    const serviceDeps = deps()
    const result = await submitPublicInquiry(command({
      inquiry: { ...INQUIRY, visitorRef: storedVisitorRef },
      trustedVisitorRef: visitorRef,
    }), serviceDeps)

    expect(result).toEqual({ idempotent: false, targetResolution: 'listing', visitorRef })
    expect(serviceDeps.createLead).toHaveBeenCalledWith(expect.objectContaining({ visitorRef }))
  })

  it.each([undefined, null])('无可信 visitorRef 时保持 null，不从 inquiry 回填：%s', async (trustedVisitorRef) => {
    const serviceDeps = deps()
    const result = await submitPublicInquiry(command({
      inquiry: { ...INQUIRY, visitorRef },
      trustedVisitorRef,
    }), serviceDeps)

    expect(result).toEqual({ idempotent: false, targetResolution: 'listing', visitorRef: null })
    expect(serviceDeps.createLead).toHaveBeenCalledWith(expect.objectContaining({ visitorRef: null }))
  })

  it.each(['', '../../etc/passwd', 'A'.repeat(32), 'a'.repeat(31)])(
    '非法 trustedVisitorRef 归一为 null，且不从 body 回填：%s',
    async (trustedVisitorRef) => {
      const serviceDeps = deps()
  ...
  it('只读预查和普通幂等提交均读回首次 Lead 的 visitorRef', async () => {
    const serviceDeps = deps({
      findExistingLead: vi.fn(async () => ({ targetType: 'building', visitorRef: storedVisitorRef })),
    })

    await expect(findExistingInquiryResult(TRUSTED_KEY, serviceDeps)).resolves.toEqual({
      targetResolution: 'building', visitorRef: storedVisitorRef,
    })
    await expect(submitPublicInquiry(command({ trustedVisitorRef: visitorRef }), serviceDeps))
      .resolves.toEqual({ idempotent: true, targetResolution: 'building', visitorRef: storedVisitorRef })
    expect(serviceDeps.createLead).not.toHaveBeenCalled()
  })

  it('唯一键竞态读回首次 Lead 的 visitorRef，不返回本次待写值', async () => {
    const serviceDeps = deps({
      findExistingLead: vi.fn().mockResolvedValueOnce(null)
        .mockResolvedValueOnce({ targetType: 'building', visitorRef: storedVisitorRef }),
  ...
  it.each([undefined, null, '../../etc/passwd', 123])('既有 Lead 缺少合法 visitorRef 时返回 null：%s', async (stored) => {
    const serviceDeps = deps({
      findExistingLead: vi.fn(async () => ({ targetType: 'listing', visitorRef: stored })),
    })

    await expect(submitPublicInquiry(command({ trustedVisitorRef: visitorRef }), serviceDeps))
      .resolves.toEqual({ idempotent: true, targetResolution: 'listing', visitorRef: null })
    expect(serviceDeps.createLead).not.toHaveBeenCalled()
  })
  ...
  it('只按服务端可信 key 预查并投影首次目标语义', async () => {
    ...
    await expect(findExistingInquiryResult(TRUSTED_KEY, deps({ findExistingLead })))
      .resolves.toEqual({ targetResolution: 'building', visitorRef: null })
