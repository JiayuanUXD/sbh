...
/**
 * 线索分析 endpoint（tasks.md M7.5 / design.md §7.5 / R6, R7）
 *
 * 路由（注册在 payload.config.ts endpoints，HTTP 前缀 /api）：
 *   - GET /leads/analytics   返回线索分析（卡 / 趋势 / 分布 + 数据截至时间）
 *
 * 业务不变量：
 ...
 *   - 与 /dashboard / /overview / /listings/analytics 一致，由 sanitizeFilters 按 metric.allowedScopeDims 收窄
 */

/**
 * 将 Payload Local API 包装为 MetricPayloadPort。
 *
 * 与 overview-endpoint / listing-analytics-endpoint 同构；保留独立导出便于后续看板扩展（如 cache 注入）。
 */
 ...
 *   ```
 */
export function createLeadAnalyticsEndpoint(): Endpoint {
  return {
    ...
    handler: async (req) => {
      ...
      // 提前拦截：无任何线索分析指标权限 → 403
      if (!canViewLeadAnalytics(permission)) {
        return Response.json(
          { ok: false, error: '无线索分析查看权限' },
          { status: 403 },
        )
      }
      ...
      // 解析线索分析（单卡失败局部标记，组间并发）
      const result = await resolveLeadAnalytics(base)
