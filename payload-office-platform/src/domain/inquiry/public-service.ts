...
import { isVisitorRef } from './visitor-ref-shape'
...
export type PublicInquiryCommand = Readonly<{
  ...
  /** 只允许 adapter 解析后注入；缺省不跟踪，不读取 inquiry.visitorRef。 */
  trustedVisitorRef?: string | null
  defaultCity: string
  siteOrigin: string
...
type ExistingLead = Readonly<{ targetType?: unknown; visitorRef?: unknown }>
...
export async function findExistingInquiryResult(
  ...
  return existing ? {
    targetResolution: targetResolution(existing.targetType),
    visitorRef: isVisitorRef(existing.visitorRef) ? existing.visitorRef : null,
  } : null
}
...
export async function submitPublicInquiry(
  ...
  deps: PublicInquiryDeps,
): Promise<ExistingInquiryResult & Readonly<{ idempotent: boolean }>> {
  ...
  const visitorRef = isVisitorRef(command.trustedVisitorRef) ? command.trustedVisitorRef : null
