...
export const Listings: CollectionConfig = {
  ...
  access: {
     ...
     */
    /**
     * 房源：绑 `listing:create` / `listing:update`（2026-09-08 收口）。
     *
     * 与上面 delete 同一个毛病、同一手法：此前这两条**也是缺省**，落到 Payload
     * 3.86 的 `defaultAccess`（判据仅 `Boolean(req.user)`）——任何登录账号
     * （经纪人、客服都算）都能新建和改写房源。UI 上那个「首页推荐」开关走的
     * 正是客户端 `PATCH /api/listings/:id`，它本身没有任何权限条件。
     *
     * `listing:create` / `listing:update` 与 `listing:delete` 一样，早就注册在
     ...
     */
    create: createCollectionAccess({ create: 'listing:create' }).create,
    update: createCollectionAccess({ update: 'listing:update' }).update,
