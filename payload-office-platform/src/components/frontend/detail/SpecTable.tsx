...
export type SpecRow = Readonly<{
  label: string
  /**
   * null 渲染为 `—`。**但正常链路上不会有 null 进来**——OPT-082 起，缺值的行在
 ...
 * **那条理由针对的是数据缺失**，它本身没有错。
 *
 * OPT-082 引入的是另一件事——**运营的编辑决策**：全站范围内决定不披露某个维度。
 * 产品裁定把两种「不显示」统一成了同一种呈现（没值就不显这行），已知代价
 * （「未勾选」与「没值」在前台不可区分、「资料不全」的信号被弱化）记录在
 * `specs/work-items/OPT-082-detail-spec-field-visibility.md` §11，是被接受的取舍，
 * 不是漏考虑。要改回去请先读那份规格，并把
 * `tests/opt082-detail-spec-hiding.test.ts` 一起改——不要只看这段注释。
 */
