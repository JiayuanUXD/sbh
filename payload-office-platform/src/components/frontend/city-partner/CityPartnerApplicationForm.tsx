  ...
  return (
    ...
    >
      ...
      {/* className="sf-num"：手机号是本表单唯一的数字输入，稿子给它单独标了
          ...
          的唯一落点是 surface.css 的 `.sf-num`，**不内联 font-variant-numeric**。
          这条会同时作用于 ComingSoonCityView 复用的那份表单——那是刻意的：
          等宽数字在任何消费面上都成立，不是招募页的局部风格。 */}
      <Field label="手机号" id="partner-phone" error={stageOneErrors.contactPhone} required><Input ref={(node) => { refs.current.contactPhone = node }} className="sf-num" type="tel" name="contactPhone" autoComplete="tel" inputMode="numeric" maxLength={20} value={stageOne.contactPhone} onChange={(event) => setStageOne((prev) => ({ ...prev, contactPhone: event.target.value }))} /></Field>
