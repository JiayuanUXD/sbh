...
export interface MediaWorkbenchItem {
  id?: string
...
const KIND_LABELS: Record<MediaWorkbenchItem['kind'], string> = {
  image: '图片',
  'floor-plan': '空间图',
  video: '视频',
}
...
/**
 * 媒体工作台的领域配置：楼盘 / 房源共用同一实现，仅以下差异需要声明。
 */
export interface MediaWorkbenchConfig {
  /** 工作台标题（如「楼盘媒体工作台」「房源媒体工作台」）。 */
  workbenchTitle: string
  /** 标题下方的功能副标题。 */
  workbenchSubtitle: string
  /** 分类键 → 中文标签；键的顺序即筛选 tab 顺序，须与 collection 的 category 枚举一致。 */
  categoryLabels: Record<string, string>
  /** 批量上传时的默认归类键。 */
  defaultUploadCategory: string
  /** 从文档数据取标题的字段名（alt 自动文案用）。 */
...
/**
 * 通用媒体工作台（绑定 Collection 的 mediaItems array 字段）
 *
 * 功能说明：
 *   1. 多文件批量拖拽 / 点击上传并自动关联媒体；
 *   2. 分类过滤与快捷筛选（按 config.categoryLabels 的键序生成）；
 *   3. 缩略图即时预览与大图弹窗；
 *   4. 一键设为封面（联动 coverImage 字段）；
 *   5. 卡片级快速切换分类、拖拽/按钮调序与批量操作。
 *
 * 楼盘（BuildingMediaManager）与房源（ListingMediaManager）仅领域文案与分类枚举不同。
 */
export default function MediaWorkbench({
  ...
  config: MediaWorkbenchConfig
  path?: string
  schemaPath?: string
  /** 与 collection mediaItems 的 maxRows 保持一致，改一处要同步另一处。 */
  maxRows?: number
}) {
  const fieldPath = path || 'mediaItems'
  const coverPath = config.coverFieldPath || 'coverImage'
  // schemaPath 缺省按 list 路径约定取 `<collection>.mediaItems`；
  // 常规挂载下 Payload 会显式传入，这里只是兜底。
  const fieldSchemaPath = schemaPath || `mediaItems`

  const { data } = useDocumentInfo()
  const { addFieldRow, dispatchFields, moveFieldRow, removeFieldRow } = useForm()
  const docTitle =
    typeof data?.[config.docTitleField] === 'string'
      ? (data[config.docTitleField] as string)
      : config.docTitleFallback

  const { value = [] } = useField<MediaWorkbenchItem[]>({ path: fieldPath })
  ...
  const docItems = useMemo<MediaWorkbenchItem[]>(() => {
    if (Array.isArray(value) && value.length > 0) return value
    if (Array.isArray(data?.mediaItems) && data.mediaItems.length > 0) {
      return data.mediaItems as MediaWorkbenchItem[]
    }
    return []
  }, [data, value])

  // 本地改动（增删改序）只累积在这里；尚无改动时回落到文档基线。
  const [localItems, setLocalItems] = useState<MediaWorkbenchItem[] | null>(null)
  const items = localItems ?? docItems
  const setItems = useCallback(
    (
      updater:
        | MediaWorkbenchItem[]
        | ((prev: MediaWorkbenchItem[]) => MediaWorkbenchItem[]),
    ) => {
      setLocalItems((prev) =>
  ...
  const currentCoverId = useMemo(() => {
    ...
    const docCover = data?.[coverPath]
    if (docCover) {
      if (typeof docCover === 'number') return docCover
      if (typeof docCover === 'object' && 'id' in docCover) return Number((docCover as { id: number }).id)
    }
    return null
  }, [coverValue, data, coverPath])

  const categoryKeys = useMemo(() => Object.keys(config.categoryLabels), [config.categoryLabels])

  const [activeCategoryFilter, setActiveCategoryFilter] = useState<string>('all')
  const [defaultUploadCategory, setDefaultUploadCategory] = useState<string>(
    config.defaultUploadCategory,
  )
  ...
  const buildRowState = useCallback((item: MediaWorkbenchItem) => {
  ...
  const handleBatchUpload = useCallback(
    async (files: FileList | File[]) => {
      ...
      const remainingRows = Math.max(0, maxRows - items.length)
      if (remainingRows === 0) {
        Message.warning(`媒体数量已达上限 ${maxRows} 个，请先删除部分媒体再上传`)
        return
      }
      ...
      const newUploadedItems: MediaWorkbenchItem[] = []
      ...
      for (let i = 0; i < fileArray.length; i++) {
        ...
        const kind: MediaWorkbenchItem['kind'] = isVideo ? 'video' : 'image'
        const category = defaultUploadCategory
        const categoryLabel = config.categoryLabels[category] ?? category
        const alt = `${docTitle} ${categoryLabel} ${items.length + i + 1}`
  ...
  const handleSetCover = useCallback(
    (resourceId: number | null) => {
      ...
      Message.success(config.coverSetMessage)
    },
    [config.coverSetMessage, setCoverValue],
  )
  ...
  const handleDeleteItem = useCallback(
    ...
    [currentCoverId, fieldPath, items, removeFieldRow, setCoverValue, setItems],
  )
  ...
  const handleCategoryChange = useCallback(
    ...
    [dispatchFields, fieldPath, items, setItems],
  )
  ...
  const handleMoveLeft = useCallback(
    ...
    [fieldPath, moveFieldRow, setItems],
  )
  ...
  const handleMoveRight = useCallback(
    ...
    [fieldPath, items.length, moveFieldRow, setItems],
  )
  ...
  const handlePointerDown = useCallback(
    ...
    [fieldPath, filteredItems, moveFieldRow, setItems],
  )
    ...
    >
      ...
      >
        <div>
          <div style={{ fontSize: 16, fontWeight: 600, color: 'var(--theme-text, #1d2129)' }}>
            {config.workbenchTitle}
          </div>
          <Text type="secondary" style={{ fontSize: 13 }}>
            {config.workbenchSubtitle}
          </Text>
        </div>
        ...
        <Space size="medium">
          ...
          <Select
            ...
            style={{ width: 140 }}
          >
            {Object.entries(config.categoryLabels).map(([k, label]) => (
      ...
      >
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {[{ key: 'all', label: `全部 (${categoryCounts.all})` }].concat(
            categoryKeys.map((key) => ({
              key,
              label: `${config.categoryLabels[key]} (${categoryCounts[key] ?? 0})`,
            })),
          ).map((tab) => {
