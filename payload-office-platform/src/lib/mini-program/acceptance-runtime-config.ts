...
export function readAcceptanceRuntimeConfig(
  env: Readonly<Record<string, string | undefined>> = process.env,
): AcceptanceRuntimeConfig | null {
  ...
  const permitSigningSecret = decodeAttestationSecret(env.MP_ACCEPTANCE_PERMIT_SIGNING_SECRET ?? '')
  const dbFingerprintAllowlist = (env.MP_ACCEPTANCE_DB_FINGERPRINT_ALLOWLIST ?? '').split(',').filter(Boolean)
  if (
    !SHA.test(deploymentGitCommitSha) ||
    !REVISION.test(deploymentRevision) ||
    !attestationSecret ||
    !operatorBootstrapSecret ||
    !permitSigningSecret ||
    Buffer.from(attestationSecret).equals(Buffer.from(operatorBootstrapSecret)) ||
    Buffer.from(attestationSecret).equals(Buffer.from(permitSigningSecret)) ||
    Buffer.from(operatorBootstrapSecret).equals(Buffer.from(permitSigningSecret)) ||
    dbFingerprintAllowlist.length === 0 ||
    dbFingerprintAllowlist.some((value) => !FINGERPRINT.test(value))
  )
    return null
  return {
