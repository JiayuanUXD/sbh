...
import { getMiniBuildingDetail } from '@/lib/mini-program/catalog-service'
...
export async function GET(
  ...
  try {
    const resolution = await getMiniBuildingDetail(city, slug)

    if (resolution.status === 'city_not_found') {
    ...
    if (resolution.status === 'building_not_found') {
      return NextResponse.json(
        miniError('building_not_found', '楼盘不存在或暂未开放', requestId),
        {
          status: 404,
          headers: { 'Cache-Control': MINI_CACHE_CONTROL, 'X-Request-Id': requestId },
        },
    ...
    console.error('getMiniBuildingDetail failed with error:', error)
    return NextResponse.json(
