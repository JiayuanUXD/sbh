...
| 编号 | 问题 | 处置 |
|---|---|---|
| **P0-1** | 上海/杭州/苏州重复城市双树，Task 21「与存量逐条比对」被绕过 | **未修，待业务方定合并策略**（见上方 P0-1 小节）。定案前禁止对生产 `--apply` |
...
| **P1-2** | 全局 `afterError` 改了**所有** collection/端点的错误契约，含公开端点 | **未修，待决策**：建议收窄为仅对已登录请求或仅挂地理相关 collection。当前实现只透传 message、不带 details，风险可控但血缘不对 |
...
| P3-1 | 全局搜索失败 `catch { setResults([]) }`，网络错误与「无结果」外观相同 | 未修（3 行改动，待办） |
...
**修复后门禁**：`pnpm test` 2656 绿（+6）· `pnpm build` 过 · `pnpm exec playwright test tests/e2e/geography-admin.spec.ts` 5/5 绿（授权路径未被守卫破坏）。
...
### ⚠️ P0-1（待决策，阻塞生产导入）：重复城市双树

本机 `sbh_dev_geo` 实测（2026-08-11 审核）：

| 城市 | 存量树 | 新导入树 |
|---|---|---|
| 上海 | id=2006 `SH` slug=`shanghai`，5 行政区 / 2 商圈 / **6 楼盘**，`frontend_visible=true` | id=3065 `CITY-SH` slug=`shanghai-new`，16 行政区 / 26 商圈 / 21 线路 / 394 站点 / **0 楼盘** |
| 杭州 | id=1001 `HZ`，1 / 1 / **1 楼盘** | id=2032 `CITY-HZ`，13 / 15 / 12 / 258 / **0 楼盘** |
| 苏州 | id=2001 `SZ`，1 / 1 / **2 楼盘** | id=2331 `CITY-SZ`，10 / 14 / 9 / 137 / **0 楼盘** |

全部 9 个楼盘挂存量树，全部 1491 条导入节点挂新树，两者零交集。Task 21 原文要求
「上海导入前必须先跑 dry-run 看冲突清单，与现有数据逐条比对，决定是跳过存量还是更新存量」——
这一步没有做。生产上后果更重：真实房源 / 线索 / 楼盘都引用存量树的行政区与商圈，
导入后「城市管理」会出现两个上海，新上海的 26 个商圈楼盘数全为 0。
