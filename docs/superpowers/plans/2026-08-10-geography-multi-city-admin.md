...
- [ ] 2 城样例数据下人工走完 A5 的两条路径，实测步数 ≤3 / ≤2。（本机无后台登录凭据，登记为人工测试项）
- [ ] A6 度量表逐项核对并记录实测值。（同上，人工测试项）
