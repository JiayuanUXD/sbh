...
| 指标 | 现状 | 目标 | 实测（2026-08-11，浏览器） |
|---|---|---|---|
| 改一个已知地理对象的操作步数 | 6~7 | ≤3 | 3（地铁管理→城市筛选→抽屉内改）；2（Cmd+K→回车） |
| 编辑一个商圈完整信息的页面跳转 | 2 页 | 1 页 | 1 页（列表打开抽屉→「完整编辑」即达含扩展面板的原生编辑页） |
| 列表首屏加载节点数 | 全库（2000 上限截断） | 单页 20 条 | 20 条（分页 pageSize=20） |
| 商圈扩展保存的地理查询次数 | `2 + 2N`（N=站点数） | 常数级 | 常数级（扩展别名保存走单次 REST PATCH，不随站点数增长） |
| 列表页计数查询次数 | —（现无计数） | 每页 **1 次聚合**，不随行数增长 | 每页 1 次聚合（Task 5 聚合服务） |
...
**验收**
- [x] 商圈编辑页可完成扩展的新建与更新，刷新后数据正确。
- [x] 跨城/停用站点候选里不出现；**用 REST 强行提交仍被后端拒绝**（不变量未被绕过）。
- [x] 两标签页并发编辑触发版本冲突并正确报错。
- [x] 非商圈类型节点不显示该面板。
...
**验收**
- [x] `tests/location-route-api.test.ts` 风格补测：命中、空串、短词不打库、跨城同名区分、未登录拒绝。
- [x] 真实 dev server 上 `curl` 通过。
- [x] 快捷键在后台任意页面可用，不与浏览器默认行为冲突（`preventDefault`）。
...
**验收**
- [x] 含 `business-areas` 与 `locations` 的存量角色登录后台，导航与角色页均无报错。（未新增/删除任何 menu code，仅复用既有 `locations`/`business-areas`，存量角色数据天然兼容——零数据迁移；实际登录点击为人工测试项）
- [x] 「基础配置」下 5 项，顺序如上。（`admin-navigation-config.test.ts` 树断言锁定 5 项顺序）
- [x] 全量 Vitest 绿（`permission-codes` 完整性测试会覆盖）。（`pnpm test` 2637 绿）
...
- 删除 `src/components/admin/LocationTreeView.tsx` 与 `LocationTreeViewClient.tsx`（**已向用户确认**：原计划「保留 LocationTreeViewClient 改造为只读供 Task 7 复用」，但实际 Task 7 城市详情页 `GeographyCityDetailClient` 已用 `location-tree` 纯函数自绘只读树，并不引用该组件；删 TreeView 后 LocationTreeViewClient 即成 100% 死代码，与「删代码要彻底」冲突，故两个都删）。
- `src/domain/geography/location-tree.ts` **不动**，其单测 `tests/location-tree.test.ts` 必须保持全绿。

**验收**
- [x] `/admin/collections/locations` 是可用的默认列表（排障入口）。（移除 `views.list` 覆盖后恢复 Payload 默认列表；config 契约已核，浏览器点击为人工测试项）
- [x] 无死代码残留（`pnpm build` 无未使用导入告警）。（构建通过无告警；`importMap.js` 已随 `generate:importmap` 重新生成清除陈旧入口）
- [x] `tests/location-tree.test.ts` 全绿且未修改。
...
**最终验收清单**
- [x] `pnpm test` 全绿（2637 用例通过）；`pnpm build` 通过（无未使用导入告警）。
- [x] `pnpm payload migrate` 从空库到最新完整跑通（scratch 库 `sbh_migrate_test` 空库→最新 `Done.`，跑完已删）。
- [x] 2 城样例数据下人工走完 A5 的两条路径，实测步数 ≤3 / ≤2。（已用 `e2e-adm@example.com` 在浏览器实测：路径一 地铁管理→城市筛选→抽屉内改 = **3 步**；路径二 Cmd+K→输「杭州2」→回车直达 = **2 步**，落地 URL `?q=HZ-METRO-L2`）
- [x] A6 度量表逐项核对并记录实测值。（见下表「实测」列）
- [x] 每个 Task 一个独立 commit，提交信息用简体中文。
...
猜错一次的返工成本，远高于问一次的沟通成本。**本计划已经在阶段一发现过"计划自身缺陷"的先例**，发现即修正即回写，是正常流程。
...
## B2.5 · 审核修复记录（2026-08-11 代码审核后）

审核发现 12 项，按严重度处理如下。

| 编号 | 问题 | 处置 |
|---|---|---|
| **P0-1** | 上海/杭州/苏州重复城市双树，Task 21「与存量逐条比对」被绕过 | ✅ 已修：定策略为**存量为准、只补差集**，种子新增 `legacyCodes` 存量别名，导入器命中后沿用存量节点。本地库已清掉重复树并按新策略重导，7 城各只剩一棵树 |
| **P0-2** | 七城导入登记表把本地 `sbh_dev_geo` 执行记为「生产执行日期」 | ✅ 已修：拆成「本地执行」「生产执行」两列，生产列全部标 ❌ 未执行，并加顶部警示 |
| **P0-2b** | 导入脚本无非本地库防护 | ✅ 已修：`--apply` 对非 localhost 库要求 `--confirm-db <库名>` 显式确认，否则 exit 2（`scripts/import-geography.ts`） |
| **P1-1** | 四个自定义 admin 路由 + 详情页 + 新建页 + 搜索端点**只判登录、不判权限**；`/new` 可越权创建行政区 | ✅ 已修：新增 `require-geography-access.tsx` 共享守卫，按模块 `menuCodes` 判定；未登录跳登录、无权限渲染 403。搜索端点补 403 分支。新增 3 个模块配置一致性测试 + 3 个端点权限测试 |
| **P1-2** | 全局 `afterError` 改了**所有** collection/端点的错误契约，含公开端点 | ✅ 已修：收窄为**只对已登录请求生效**，`req.user` 为空直接返回 undefined，公开端点（inquiries / supply-submissions / corrections）对匿名调用方的契约一个字不变。新增 4 例单测 |
| **P2-1** | `business_area_extensions_rels` join 漏 `path` 过滤（现在算对，加第二个 hasMany 就静默算错） | ✅ 已修：补 `rels.path = 'metroStations'` + 文件头说明 |
| **P2-2** | 楼盘计数注释称「照搬 public-building」，实际少了 `city.status` / `district.status` 两条 | ✅ 已修：改为明确说明**有意不带**这两条（停用行政区时楼盘数不应归零），并写明本文件计数 ≥ C 端可见数 |
| **P2-3** | 七城种子文件头 `采集日期：待回填`、`核对人：<name>` 全是占位符 | ✅ 已修：采集日期填 2026-08-11；核对人如实标注「未经人工核对，由实现代理生成，待业务方复核后回填」 |
| P3-1 | 全局搜索失败 `catch { setResults([]) }`，网络错误与「无结果」外观相同 | ✅ 已修：新增 error 态，同时补 `res.ok` / `data.ok` 判定（原实现连 401/403/500 都渲染成「无匹配结果」），失败以红字提示真实原因 |
| P3-2 | `intArrayLiteral` 遇非数字 id 产生 `{NaN}` → PG 语法错而非受控失败 | 未修（当前 ids 均来自 payload 查询，风险低） |
| P3-3 | `fetchBusinessAreaMissingBoundaryIds` 全库扫描不带城市过滤 | 未修（2000 条规模无碍） |
| P3-4 | 表格所有列均不可排序（超额满足「计算列不可排序」） | 未修（产品决策，待确认是否要放开字段列排序） |

**追加发现（审核第一轮漏掉的）**：`--all` 参数一直是坏的——`readdirSync` 只返回文件名，
`seedFiles` 未拼目录，跑 `--all` 会七个文件全部「文件不存在」。此前的分城导入都是逐个
`--file` 执行的，所以没暴露。已修。

**修复后门禁**：`pnpm test` 2667 绿（+17）· `pnpm build` 过 · `pnpm exec tsc --noEmit` 过 ·
`playwright test geography-admin` 5/5 绿。
...
| 13 全局搜索（Cmd/Ctrl+K） | ✅ 完成 | `feat(geography): Task 13 全局搜索`；endpoint `GET /api/locations/search?q=&limit=`（挂 **collection endpoints**，避免 slug 路由遮蔽 404）：`searchLocations` 纯函数按 `name`/`immutableCode` `contains` 命中、`q` 去空格后 <2 **不打库**直返空数组、`overrideAccess:false` 继承当前用户数据权限、limit 上限收敛 50；未登录 401。**对 Part B 响应形状做最小调停**：计划体的 `{id,name,type,cityId,cityName,parentName}` **不含 immutableCode**，但计划自己的 UI 示例要展示代码（`METRO-HZ-L2`）且导航要 `?q=<code>`——故结果里**补 `immutableCode`**（计划 UI 倒逼的最小加减）。导航映射 `locationSearchTarget` 纯函数：city→`/geography/cities/:id`（Task 7 详情页）、district/business_area/metro_line→对应模块列表 `?q=<code>`、metro_station（无模块）→ Payload 原生编辑页 `/admin/collections/locations/:id`。前端 `GeographyQuickSearch.tsx`（**挂 `admin.components.actions`** 才常驻监听）Cmd/Ctrl+K 唤起（`preventDefault` 防浏览器冲突）、Esc 关、上下键导航、回车跳转，结果按城市分组 + 类型标签 + 代码。单测 13 例。C2 门禁：typecheck 过、`pnpm test` 2637 绿、`pnpm build` 过、无 schema 变更（免 migrate）；REST 验收：命中（龙翔→站/杭州→城）、短词空数组不打库、未登录 401、limit 收敛 50 均过。注：in-app 浏览器预览不可用 + 无后台登录凭据，Cmd/Ctrl+K **交互**未做真实浏览器点击验证（编译 + REST 契约已验证），已在回报中说明 |
| 14 编辑页按类型收敛字段 | ✅ 完成 | `feat(geography): Task 14 编辑页按类型收敛字段`；纯 admin 配置，无 schema 变更（免 migrate）。`type`/`immutableCode` 加 `admin.readOnly:true`（**已向用户确认**：本版 Payload 3.86 `readOnly` 只支持 boolean，无函数形式，无法「编辑态只读/创建态可编辑」；恒只读合理——所有真实新建走自定义视图(Task 8)/内嵌面板(Task 12)直接 POST REST，不经原生表单，原生新建页无法填这两项无实际影响，`admin.readOnly` 不影响 REST/Local API 写入）；`parent` 加 condition `type!=='city'`（城市无上级）；`description`/`coverImage` 加 condition `business_area||district`；`centerLatitude/Longitude` 全部类型恒定显示（不改）；`city`、两个 ui 面板条件维持原样。C2 门禁：typecheck 过、`pnpm test` 2637 绿、`pnpm build` 过。注：in-app 浏览器预览不可用 + 无后台登录凭据，各类型编辑页「只出现相关字段 / 只读字段不可改」的**交互**未做真实浏览器点击验证（编译 + 配置契约已核），登记为人工测试项。**2026-08-11 已用 `e2e-adm@example.com` 浏览器实测补全**：城市（上海）页隐藏 上级区域/区域介绍/封面图/商圈扩展/线路站点 ✓；地铁线路页显示 上级区域+线路站点、隐藏 区域介绍/封面图/商圈扩展 ✓；商圈（陆家嘴）页显示 上级区域/区域介绍/封面图+商圈扩展、隐藏 线路站点 ✓；`type`/`immutableCode` 输入框 disabled 不可改 ✓。 |
| 15 导航改造与权限编码 | ✅ 完成 | `feat(geography): Task 15 导航改造与权限编码`；`navigation-config.ts` 的「基础配置」子分组 3 项→5 项：`cities`(城市管理, `/geography/cities`)、`districts`(行政区域, `/geography/districts`)、`business-areas`(商圈管理, `/geography/business-areas`)、`metro-lines`(地铁管理, `/geography/metro-lines`)、`amenities`(配套字典, 原样)。**权限数据零迁移**：四个地理模块复用既有 menu code `locations`/`business-areas`，未新增/删除任何 code（存量角色数据天然兼容）。四个 href 非 `/admin/collections/:slug` 形式，`leaf()` 推导 collectionSlug 为 undefined；已读 `navigation-types.ts`+消费方确认下游 `resolve-navigation.ts:84` 依赖 `leaf.collectionSlug` 做读权限过滤 → 四个地理叶子**显式传 `{ collectionSlug: 'locations' }`**（`locations` 与 `business-area-extensions` 均 `read:()=>true`，对登录用户等价）。测试：`admin-navigation-config.test.ts` 树断言 3→5 项、`admin-navigation-visibility.test.ts` 递归移除断言 `['supply','supply-settings','locations']`→`['supply','supply-settings','cities','districts','metro-lines']`；`role-matrix`/`state`/`payload-config` 三文件不受影响（局部 fixture / 只查 collection 配置 / 不依赖 collectionSlug）。C2 门禁：typecheck 过、`pnpm test` 2637 绿、`pnpm build` 过、无 schema 变更（免 migrate）。注：存量角色登录后 nav 与角色页无报错为人工测试项（数据兼容已由「不动 code」保证） |
| 16 旧树视图下线 | ✅ 完成 | `feat(geography): Task 16 旧树视图下线`；`Locations.ts` 移除 `admin.components.views.list`（LocationTreeView 覆盖）→ 恢复 Payload 默认列表作排障兜底；`labels.plural` `'行政区域'`→`'地理数据'`（面包屑用，不再出现在导航）。**对原计划做一处调停（已向用户确认）**：计划要「保留 LocationTreeViewClient 改造为只读供 Task 7 复用」，但实际 Task 7 城市详情页 `GeographyCityDetailClient` 已用 `location-tree` 纯函数（`buildChildrenIndex`/`groupCityDirectChildren`）自绘只读树、并不引用该组件；删 `LocationTreeView.tsx` 后 `LocationTreeViewClient` 即成 100% 死代码，与计划「删代码要彻底」冲突 → 经 AskUserQuestion，用户拍板**两个都删**。`importMap.js` 随 `pnpm exec payload generate:importmap` 重新生成，清掉两个陈旧入口（diff 仅删 2 行）。`location-tree.ts` 未动、`tests/location-tree.test.ts` 未改仍全绿；`admin-navigation-payload-config.test.ts` 的 locations plural 断言 `'行政区域'`→`'地理数据'` 同步更新。C2 门禁：typecheck 过、`pnpm test` 2637 绿、`pnpm build` 过（无未使用导入告警）、无 schema 变更（免 migrate）。注：`/admin/collections/locations` 默认列表可用为人工测试项；`GET /api/locations/:id/references` endpoint 保留（其域名函数 `countLocationReferences` 仍被删除守卫 `location-delete-guard` 用），本任务不删 |
...
| 17 E2E 与最终验收 | ✅ 完成 | `feat(geography): Task 17 E2E 与最终验收`；新增 `tests/e2e/geography-admin.spec.ts`，5 条流程（`describe.serial` + `beforeAll` 按 slug 幂等灌入 **spec 专属唯一城市「全域测试城」** fixture，计数可硬编码、空库/开发库全环境确定性一致、不破坏既有样例）。**E2E 暴露并修复一个真实 bug**：`GeographyListViewClient.tsx` 的 `handleCityChange` 拆两次 `setFilter`（先 set city 再重置 parent），第二次复用旧 `searchParams` 闭包 `router.push(pathname)` 把刚写入的 `city` 覆盖回无参 URL → 城市筛选完全失效（选城后 URL 不变、行不筛）。改为**单次 URL 更新**（同一份 searchParams 同时 set city / delete parent / delete page）。E2E 全 5 流程绿且幂等（连续两轮通过）；另 `getByDisplayValue` 在本 Playwright 构建不可用 → 改 `input[value=]` 定位。C2 门禁：`pnpm test` 2637 绿、`pnpm build` 过、`pnpm payload migrate` 空库→最新 `Done.`（scratch 库 `sbh_migrate_test`，跑完已删）。待人工（本机无后台登录凭据 + in-app 浏览器预览不可用）：A5 两条路径实测步数 ≤3/≤2、A6 度量表实测值。**2026-08-11 已用 `e2e-adm@example.com` 浏览器实测补全**：A5 路径一 3 步、路径二 2 步（Cmd+K→「杭州2」→回车直达 `?q=HZ-METRO-L2`）✓；A6 各指标实测值见 A6 表「实测」列，均达目标 ✓。 |

**七城导入登记**（Task 21 逐城填写）

> ✅ **生产库已于 2026-08-12 完成导入**，执行记录见交接文档
> [2026-08-11-geography-production-import-handoff.md](2026-08-11-geography-production-import-handoff.md)。
> 下表「本地执行」列是本机开发库 `sbh_dev_geo` 的历史记录（早于 P0-1 存量策略定案，
> 六城为纯新增口径）；「生产执行」列是按「存量为准、只补差集」策略在生产 TencentDB
> 的实际写入结果，两列口径不同不能直接相加（尤其上海——生产是存量沿用为主，见下方说明）。

| 城市 | 行政区 | 商圈 | 线路 | 站点 | dry-run 日期 | 本地执行（sbh_dev_geo） | 生产执行 |
|---|---|---|---|---|---|---|---|
| 杭州 | 13 | 15 | 12 | 258 | 2026-08-10 | 2026-08-10 | ✅ 2026-08-12，新建 299（1城+13区+15商圈+12线+258站），沿用0，冲突0，失败0 |
| 苏州 | 10 | 14 | 9 | 137 | 2026-08-10 | 2026-08-10 | ✅ 2026-08-12，新建 171（1+10+14+9+137），沿用0，冲突0，失败0 |
| 无锡 | 7 | 12 | 4 | 71 | 2026-08-10 | 2026-08-10 | ✅ 2026-08-12，新建 95（1+7+12+4+71），沿用0，冲突0，失败0 |
| 宁波 | 10 | 12 | 8 | 170 | 2026-08-10 | 2026-08-10 | ✅ 2026-08-12，新建 201（1+10+12+8+170），沿用0，冲突0，失败0 |
| 嘉兴 | 7 | 10 | 0 | 0 | 2026-08-10 | 2026-08-10 | ✅ 2026-08-12，新建 18（1+7+10+0+0，该市无地铁），沿用0，冲突0，失败0 |
| 南京 | 11 | 19 | 12 | 206 | 2026-08-10 | 2026-08-10 | ✅ 2026-08-12，新建 249（1+11+19+12+206），沿用0，冲突0，失败0 |
| 上海（Wave 1：地铁） | — | — | 21 | 394 | 2026-08-10 | 2026-08-10 | ✅ 2026-08-12，新建 415（21线+394站），沿用存量 1（城市节点，见下方说明），冲突0，失败0 |
| 上海（Wave 2：行政区/商圈） | 19 | 26 | — | — | 2026-08-11 | 2026-08-10 | ✅ 2026-08-12，新建 6（虹桥商务区/漕河泾开发区/临港/长风商务区/大虹桥/松江新城），沿用存量 37（1城+16区+20商圈），跳过 421（Wave1已导地铁），冲突0，失败0 |
| **合计（六城，纯新增）** | **74** | **108** | **66** | **1236** | | | **新建 1433** |
| **合计（上海，两波）** | | | | | | | **新建 421，沿用存量 38** |
| **合计（七城，两波）** | | | | | | | **新建 1854，沿用存量 38，冲突 0，失败 0** |

> **生产执行记录（2026-08-12）**：按交接文档分两波执行，全程用 CloudBase MCP 只读查询做验收核对（未用 SQL 直接写库）。
>
> - **Wave 1**（六城全量 + 上海地铁 `--only metro`）：合计新建 **1448** 个节点，0 冲突 0 失败，六城「沿用存量」均为 0（符合预期，生产此前没有这些城市）。上海城市节点触发**多命中告警**——`CITY-SH` 的 `legacyCodes` 同时命中 `LEGACY_LOC_1`(id=1，启用，挂 71 楼盘) 与 `SH`(id=6，停用)，脚本按声明顺序取首个 `LEGACY_LOC_1`，人工核对**正确**（未挂到停用空壳上）。
> - **Wave 2**（上海行政区与商圈全量，无 `--only`）：合计新建 **6**（虹桥商务区/漕河泾开发区/临港/长风商务区/大虹桥/松江新城）、沿用存量 **37**（1 城 + 16 区 + 20 商圈）、跳过 **421**（Wave 1 已导的地铁）、冲突 0、失败 0。逐条核对 5 条关键别名全部命中正确：`CITY-SH→LEGACY_LOC_1`、`SH-D-310106（静安区）→LEGACY_LOC_2`、`SH-D-310115（浦东新区）→LEGACY_LOC_3`、`SH-BA-NANJINGXILU→LEGACY_LOC_4`、`SH-BA-LUJIAZUI→LEGACY_LOC_5`。未认领存量节点 189 条 = 3 条生产原有的停用重复行政区空壳 + 186 条种子未覆盖的生产自有商圈（206 商圈−20 条被认领=186，数字自洽），逐条核对无「本该认领却漏掉」的情况。
> - **验收 SQL 实际返回值**：孤儿节点 `city_id IS NULL` = 0；城市数 = 8（无第三条上海）；上海地铁 21 线/394 站；`frontend_visible=true` 节点数 = 26（导入前基线，未增加）；上海行政区数 = 19（未新建）；上海商圈数 = 206→212；`buildings` 关联数 = 71（未受影响）；重复行政区名仅剩生产原有的停用空壳（浦东新区×2、静安区×2），无新增重复。
> - **执行中的意外**：生产 TencentDB 连接在长任务中多次中断（`Connection terminated unexpectedly`），杭州/南京/宁波三城的首次 `--apply` 中途断连，依赖脚本的 `immutableCode` 幂等特性逐城重跑补齐，最终逐城核对新建数与 dry-run 预期完全一致。上海 Wave 2 的 `--apply` 也在写完 6 个商圈后于收尾报告阶段断连，经 DB 直查确认 6 个商圈已完整写入、幂等复跑确认「新建 0」后判定完整。**过程中没有执行任何 SQL 直接写库，也没有使用 `--update-existing`**，符合交接文档红线。
> - **日志文件**：`.tmp/geography-import-*.log`（每次调用一份，本次两波共产生约 15 份，因多次断连重跑）。
>
> 以下为该表格历史记录（本机开发库 `sbh_dev_geo` 的早期导入，早于 P0-1 存量策略定案，口径与生产不同，仅作参考）：
>
> 导入口径：`scripts/import-geography.ts --apply`，幂等键=immutableCode；全部 `frontendVisible=false`（C 端不可见，Task 22 不批量开启）。7 城合计 7 城记录+74 行政区+108 商圈+66 线路+1236 站点 = **1491 条，0 冲突 0 失败**。导入后校验：`type<>'city' AND city_id IS NULL` = 0（无孤儿）；E2E 专用 7 条 fixture（`E2E-*`）保留未动。**slug 冲突处理 → 遗留为 P0-1，未解决**：旧 HZ/SZ/SH 城市为 C 端在线存量、须保规范 slug（`hangzhou`/`suzhou`/`shanghai`），新导入城市 CITY-HZ/SZ/SH 改了过渡 slug（`hangzhou-new`/`suzhou-new`/`shanghai-new`）落库。**这不是冲突的解决，是绕开**——结果是同一城市存在两棵互不相交的树（见下方「P0-1」）。旧格式 24 条遗留记录：10 条叶子节点在**本机开发库**被物理删除，14 条被 `protectLocationDelete` 守卫拦截保留（被开发楼盘/商户节点引用，只能「停用」不能删），符合删除守卫预期。

### ✅ P0-1（已定案并落地）：存量为准、只补差集

审核发现同一城市存在两棵互不相交的树（存量树挂着全部业务引用，新导入树 0 楼盘）。
成因是导入按 `immutableCode` 精确匹配，而存量节点用的是旧编码（如上海静安区存量码
`SH-JINGAN`，本规范是 `SH-D-310106`），于是同一个现实对象被当成新节点再建一遍，
且**静默成功、不报错**。

**已定策略：存量为准、只补差集。** 实现方式：

1. 种子节点新增 `legacyCodes` 字段，显式声明该节点对应的存量旧编码。
   刻意要求人工声明而非按名称模糊匹配——地理节点改名会波及所有引用它的房源/线索/楼盘。
2. 导入器先按 `immutableCode` 匹配，未命中再按 `legacyCodes` 匹配；命中别名即
   **沿用存量节点**（不改名、不改码、不改 slug），只把它作为下级的父级。
3. 每城导入结束反查「未被种子认领的存量节点」并给出同名疑似对应提示 —— 这是本策略的
   审计面：漏声明 `legacyCodes` 的后果是静默建重复点，必须让它显形。

**已声明的存量别名**（本地库实测认领 14 个节点）：

| 城市 | 认领 |
|---|---|
| 上海 | `SH`→CITY-SH · `SH-HUANGPU/XUHUI/CHANGNING/JINGAN/PUDONG`→对应 `SH-D-*` · `SH-JINGAN-NJW`→南京西路 · `SH-PUDONG-LJZ`→陆家嘴（8 个） |
| 杭州 | `HZ`→CITY-HZ · `HZ-XH`→西湖区 · `HZ-QJ`→钱江新城（3 个） |
| 苏州 | `SZ`→CITY-SZ · `SZ-GS`→姑苏区 · `SZ-JJH`→金鸡湖（3 个） |

**本地库重导结果**（先清掉 928 条重复树节点，再按新策略 `--all --apply`）：

| 城市 | 行政区 | 商圈 | 线路 | 站点 | 楼盘 |
|---|---|---|---|---|---|
| 上海 (SH, id=2006) | 16 | 26 | 21 | 394 | 6 |
| 杭州 (HZ, id=1001) | 13 | 15 | 12 | 258 | 1 |
| 苏州 (SZ, id=2001) | 10 | 14 | 9 | 137 | 2 |
| 南京 / 宁波 / 无锡 / 嘉兴 | 11 / 10 / 7 / 7 | 19 / 12 / 12 / 10 | 12 / 8 / 4 / 0 | 206 / 170 / 71 / 0 | 0 |

每城单树、楼盘与导入数据同挂一棵树、孤儿节点 0；幂等复跑 0 新建。

**生产导入前仍须**：先 dry-run 看「未认领存量节点」清单，为其中真正对应的节点补
`legacyCodes` 再执行——生产的存量集合与本地开发库不同，别假设这 14 个别名就够。

<details><summary>当时评估过的三个候选方案（存档）</summary>

**三个候选方案（需业务方拍板，本期未定）**：
1. 把存量节点的 `immutableCode` 对齐到新规范后合并（一次性改主键语义，风险最高）。
2. **存量为准，新导入只补差集**（推荐）：跳过已存在的城市/行政区，只导入存量缺失的商圈、线路、站点，挂到存量城市下。
3. 接受双树，明确废弃旧树并迁移全部业务引用（工作量最大，但结构最干净）。
