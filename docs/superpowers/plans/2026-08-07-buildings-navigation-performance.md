...
**Architecture:** 在现有前台缓存查询模块中增加无参数的楼盘搜索缓存封装，复用 Public Catalog `searchBuildings` 结果；缓存同时依赖楼盘和房源类别标签，并以 300 秒重新验证限制陈旧窗口。楼盘列表 Server Component 继续负责 URL 筛选和内存分页，只把直接 Facade 调用替换为缓存封装。
...
describe('OPT-025 楼盘列表导航性能合同', () => {
  it('楼盘搜索缓存同时依赖楼盘与房源，并最多缓存 300 秒', async () => {
