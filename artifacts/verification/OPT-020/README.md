...
- GitHub Actions `30258894609` 进一步确认 CLI 上传在约 5 分钟后因 `ECONNRESET` 失败；CloudBase 没有创建版本，线上未切流。
- 显式灰度版本 `sbh-010` 构建时发现 Dockerfile 复制不存在的 `/app/public`；版本保持 0% 流量。
- 已增加 Docker 构建上下文与显式灰度参数回归测试，并修复空 `public` 目录处理。
- 工作流已改为官方 API 分配上传地址，通过 `curl` HTTP/1.1 明确内容长度、设置单次超时并重试 4 次，随后显式提交 `GRAY`；部署记录到 `normal` 后才切 10% 流量。
- 灰度冒烟允许命中旧版健康接口时返回 404，但必须在 50 次请求内至少命中一次状态正常的新版本；全量后重新验证健康接口、首页和后台。
