...
**typecheck 干净 + 单测全绿 + CI 三项通过 ≠ 可用。** 声称"完成/已验证"之前，
用浏览器把改动实际点一遍。

真实教训（2026-08-26，OPT-053）：后台新增的「站点设置」入口，菜单渲染正常、
点进去是 Payload 的「没有找到任何东西」。当时 `typecheck` 干净、**3823 个单测全绿**、
`next build` 通过、CI 三项全绿、C 端 curl 逐项验过——**只有真的点那个菜单才会发现**。
根因是 Global 写了 `admin.hidden: true`（会连路由一起排除，应该用 `group: false`）。

### 怎么走

| 场景       | 做法                                                                               |
| -------- | -------------------------------------------------------------------------------- |
| C 端页面    | `preview_start` 起 dev server，用 `mcp__Claude_Browser__*` 点、读、验                    |
| **后台页面** | 同上，用 `scripts/seed.ts` 里的 E2E 夹具账号登录（`e2e-adm@example.com` 等，密码在该文件里，是公开夹具不是真凭据） |
| 生产环境的后台  | 需要用户的登录态，走 `mcp__claude-in-chrome__*`；**agent 不得输入密码**                           |

### 两条铁律

1. **验不到就明说验不到**，不要用"CI 全绿"顶替。生产后台需要登录态而当前拿不到时，
   如实说明并请用户配合，而不是跳过后宣称已验证。
2. **本地验之前先确认环境等价**：本地库常常落后于 `src/migrations/`
   （跑 `pnpm exec payload migrate`），否则会看到"缺列导致 500 → 页面降级"这类
   与代码无关的假象，把结论带偏。真实教训：同一天我因此一度推翻了一个正确的诊断。

### 做对照实验时

**先确认对照组真的是未修复状态。** 我曾拿同仓库里另一个 Global 当"未修复对照"，
而当时跑的工作树已含修复，对照无效；叠加上一条的环境差异，导致结论完全反了。
