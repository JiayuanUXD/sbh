...
- **不在 `master` 上直接写代码**（push master 会触发生产部署）。先从最新 `origin/master` 开分支，命名为 `<类型>/<kebab 描述>-<4~8位随机hex>`（如 `feat/opt-022-dashboard-perf-a3f1`）；可用 `cd payload-office-platform && pnpm branch:new feat <描述>` 自动生成。
