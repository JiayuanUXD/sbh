...
- **不在 `master` 上直接写代码**（合并到 `master` 会在闸门通过后自动全量上线，没准备好就别合，见 `CLAUDE.md`；平台侧构建 / 版本创建失败时 deploy job 直接红、不切流，处置见 `DEPLOYMENT.md`「发布（CI）」）。先从最新 `origin/master` 开分支，命名为 `<类型>/<kebab 描述>-<4~8位随机hex>`（如 `feat/opt-022-dashboard-perf-a3f1`）；可用 `cd payload-office-platform && pnpm branch:new feat <描述>` 自动生成。
