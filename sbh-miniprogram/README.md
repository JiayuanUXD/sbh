...
这是 SBH 商办租赁项目的原生微信小程序工程，与 Web 应用共用仓库，但拥有独立的依赖、锁文件和微信项目配置。MP-103 已把首页、搜索与房源列表接成首个匿名用户业务闭环：首页搜索或真实快捷筛选会进入找房页，列表按当前计价单位独立展示并覆盖加载、空、错误、刷新和分页状态。

## 业务入口与范围

- `pages/home/index` 是应用首路由，`pages/listings/index` 是找房列表。原 `pages/foundation/index` 仅作为工程基础诊断页保留，不在 tabBar 中。
- 当前 tabBar 只有“首页”和“找房”两项。楼盘和“我的”属于 MP-105/MP-106 后续范围，本阶段不创建无功能空页。
- 房源点击只保留 MP-104 详情导航边界；当前不实现收藏、地图、咨询写入或伪造详情页。
...
CLI 文件必须具有执行权限。脚本会先校验 CLI，再加载自动化依赖；它以可信工程模式打开当前工程，先 `reLaunch('/pages/home/index')` 并等待 `#home-ready`，再 `switchTab('/pages/listings/index')` 并等待 `#listings-ready`。启动、两次路由、两个 ready、每页验收窗口与关闭都有超时，任一页验收窗口内的运行时异常都使结果失败。成功或失败都会尝试关闭连接，关闭失败时再安全断开；即使启动已超时，迟到建立的连接也会尝试被回收。失败入口会再给这些 best-effort 清理 250ms 宽限，随后强制以退出码 1 结束，避免官方 automator 遗留活跃句柄让 Node 无限悬挂；成功路径不强制退出。它不预览、上传或部署代码。

如果 IDE 服务端口关闭、本机尚未登录，或其他微信环境条件不满足，应将模拟器自动化记录为“未执行 + 具体原因”。Node 测试不能替代开发者工具或真机验收。MP-103 当前证据在 `artifacts/verification/MP-103/README.md`。
