...
这是 SBH 商办租赁项目的原生微信小程序工程，与 Web 应用共用仓库，但拥有独立的依赖、锁文件和微信项目配置。当前 foundation 页面用于验证工程、设计 token、基础组件与自动化链路，不是业务首页。
...
CLI 文件必须具有执行权限。脚本会先校验 CLI，再加载自动化依赖；它以可信工程模式打开当前工程，对启动、路由、ready、验收窗口与关闭分别设置超时，严格核对 `pages/foundation/index`、轮询 `#foundation-ready`，并把验收窗口中的任意运行时异常视为失败。成功或失败都会尝试关闭连接，关闭失败时再安全断开；即使启动已超时，迟到建立的连接也会被回收。它不上传代码。如果本机尚未安装或登录微信开发者工具，应将该项记录为“未执行”，不能用 Node 测试冒充通过。
