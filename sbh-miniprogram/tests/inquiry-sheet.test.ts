...
describe('inquiry-sheet 组件运行时与视觉合同', () => {
  ...
  it('真实打开态使用共享骨架、分段手机号选择和始终可见的安全区主动作', () => {
    const markup = readFileSync(resolve(sheetRoot, 'index.wxml'), 'utf8')
    const styles = readFileSync(resolve(sheetRoot, 'index.wxss'), 'utf8')
    const bodyEnd = markup.indexOf('</scroll-view>')
    const footerStart = markup.indexOf('class="inquiry-sheet__footer sbh-sheet__footer"')

    expect(markup).toContain('class="inquiry-sheet sbh-sheet"')
    expect(markup).toContain('class="inquiry-sheet__backdrop sbh-sheet__backdrop"')
    expect(markup).toContain('class="inquiry-sheet__panel sbh-sheet__panel"')
    expect(markup).toContain('inquiry-sheet__grabber')
    expect(markup).toContain('inquiry-sheet__phone-segment')
    expect(markup).toContain("snapshot.phoneMode === 'wechat'")
    expect(markup).toContain("snapshot.phoneMode === 'manual'")
    expect(markup).toContain('class="inquiry-sheet__body sbh-sheet__body"')
    expect(footerStart).toBeGreaterThan(bodyEnd)
    expect(markup).toContain("snapshot.state === 'success'")
    expect(markup).toContain("snapshot.state === 'submitting' ? '正在提交' : '提交咨询'")
    expect(styles).toContain('var(--sbh-sheet-panel-radius)')
    expect(styles).toMatch(/\.inquiry-sheet__close\s*\{[\s\S]*width:\s*var\(--sbh-sheet-close-size\);[\s\S]*height:\s*var\(--sbh-sheet-close-size\);/)
    expect(styles).toMatch(/\.inquiry-sheet__footer\s*\{[\s\S]*env\(safe-area-inset-bottom\)/)
  })
})
