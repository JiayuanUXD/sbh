...
2. 合并顺序有依赖，方向是 **#47 先、#46 后**：#46 的分支不含本工作项的种子修复，
   单独跑 e2e 必红（详情页全量 404），且会被它自己新加的 timeout-minutes: 30 掐掉。
   #47 单独合是自洽的——多城市用例会 test.skip 跳过，主闸门能绿。
...
**PR #46（后置：必须在 #47 合并之后再 rebase 重跑）**
...
- [ ] #47 合并后，把 #46 rebase 到新 master 再重跑（顺序不能反：#46 缺种子修复，单独跑必红）
- [ ] #46 重跑时，那三个多城市 spec 会**第一次以开启态真正执行**——它们从未在任何环境通过过，很可能暴露新失败，这正是 #46 的价值
- [ ] #43 的四条人工验收（§9）
- [ ] #45 合并前的生产数据变更确认
