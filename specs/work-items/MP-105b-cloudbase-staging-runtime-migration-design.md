...
>
> 状态：迁移设计已落地，当前基线为新环境 `sbhmini-003=100%`。004 的一次性部署、结果未知处理与恢复协议以 MP-105 当前计划为准；本设计中的迁移期创建/回滚叙述不得作为 004 自动 mutation 指令重放。
...
旧 env: sbhmini-d5g7d6732b2c64a66
  ...
  -> 旧 sbhmini CloudRun：只读保留，不作为 004 自动回滚或 mutation 目标
```
...
- 数据库连接失败时冻结 trial 并只读核对新环境服务/数据库身份；不由一次性部署脚本自动回滚、重试、修改或清理旧 PostgreSQL。
- 004 结果未知时停止生成/上传 trial，只运行独立只读 reconciler；不得自动撤销关联、重放发布或把旧服务作为 mutation 回滚入口。后续人工处置必须基于精确平台状态另行授权。
- 写验收必须使用 run-scoped permit、恢复回执、幂等对账和精确清理；结果未知或进程中断时保留本地恢复胶囊，禁止普通重试或无条件 `finally` 清理，改由独立 recovery CLI 在旧 writer 到期后完成同锁对账。
