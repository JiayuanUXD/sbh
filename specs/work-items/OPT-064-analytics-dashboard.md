...
> ⚠️ **编号 065 事实上被撞了**：2026-09-02 另有 `OPT-065-site-favicon.md` 占用同号，
> master 上两批 `OPT-065 ...` 提交交错（数据看板页 vs 站点图标）。两边都已上线，
> renumber 会让文件与自己的提交信息对不上，**故就地记录、065 号作废不再复用**，
> 下一个新编号从 **068** 起。同类前科：OPT-058、OPT-061。
...
> ✅ **已上线（2026-09-03，PR #138 / `9f0ea76`）**。生产库已核验：`leads.visitor_ref`
> 列在、`leads_visitor_ref_idx` 索引在、`20260903_084339_opt_067_lead_visitor_ref`
> 记在 `payload_migrations`。**这条只能在生产查**——本地库怎么跑都证明不了生产
> 迁移执行过，而本工作项恰好两次漏提交迁移的配套文件（§12-11）。
> 实施与本节的差异见 §12-9、§12-10。
...
| 7 | ~~隐私政策文案过目~~ | ✅ **已闭环（2026-09-03）**：用户明确表示不需要逐字过目。条款分两次上线——OPT-064a 的「匿名访问统计」、OPT-067 的「提交后关联」，版本号随之升到 `MVP-R2`。升版本的代价（部署窗口期拒绝在途提交）是有意选的，见 §12-9 |
...
### 12-9 隐私政策升到 MVP-R2：部署窗口期会拒绝在途提交，这是有意的

服务端对 `consent.policyVersion` 是**精确匹配**（`inquiry/schema.ts:262`）。
把版本从 `MVP-R1` 升到 `MVP-R2`，意味着部署那一刻：**在旧页面上填好、
部署后才点提交的用户会被 422 拒绝**，得刷新重填。

放行 R1 更「友好」，但那等于**在用户同意的是 R1 文案的前提下，执行 R2 才描述的
行为**——R1 原文承诺「无法用于跨天或跨设备识别您本人」，而 R2 才补上「提交
咨询之后……会把本次访问的匿名浏览记录与该条咨询关联起来」。放行 = 做了用户
没同意的事，且事后无从分辨哪条线索属于哪种情形（`consent.policyVersion`
存的是客户端报上来的值）。

**取舍**：宁可让部署窗口那几分钟内的极少数用户重填一次，也不要一批
「同意的是 R1、却按 R2 处理」的线索。用户已认可。

附带教训：E2E spec 不走 `@/` 别名，只能把版本号抄一份，升版本时漂了一次——
**所有提交表单的用例集体 422，而本地闸门（typecheck + test）完全看不见**。
已补 `tests/privacy-policy-version-sync.test.ts` 把它拉回单测层，几百毫秒就红。

### 12-10 §6.4 的两处实施差异

1. **形状校验单独拆一个文件**。§6.4 只说「严格校验 32 hex」。实施时
   `visitor-ref.ts` 要 `import { createHmac } from 'node:crypto'`，而客户端
   （`visitor-identity.ts`、`LeadVisitorPathLink.tsx`）校验的是同一个形状——
   直接 import 会把 `node:crypto` 拖进浏览器 bundle。于是把
   `VISITOR_REF_LENGTH` / `isVisitorRef` 拆到 `visitor-ref-shape.ts`，
   服务端侧再 re-export，口径仍只有一个来源。

2. **缺 `PAYLOAD_SECRET` 时跳过派生，不抛错**。`deriveVisitorRef` 本身缺密钥
   必须抛（不能静默降级成可反推的普通哈希），但**调用点不能让它冒泡**：
   第一版把 `resolveVisitorRef` 写在两个 try 之外，
   于是没有该环境变量时 `/api/inquiries` **整个 500**——31 个单测当场红。
   优先级搞反了：analytics 是附加品，线索是核心业务，**分析侧的任何问题都
   不得阻断留资**。现在缺密钥只记一条 `inquiry_visitor_ref_skipped` 警告日志。
   （生产有 `config-guard` fail-closed 保证该变量存在，这条兜的是本地与测试。）

### 12-11 `migrate:create` 产出三个文件，显式 `git add` 会只加走一个

本工作项**同一个坑踩了两次**：先漏 `src/migrations/index.ts` 的注册项
（开 PR 前 `git status` 才发现），再漏 `.json` 快照（CI 的 `migrate:drift` 抓到）。

为什么两道闸门都没拦住：

- `.githooks/pre-commit` 第 4 项只判**有没有暂存一个
  `src/migrations/<数字>*.ts`**，`.ts` 加了就算过——它防的是「改了 collection
  却完全没建迁移」，不是「建了但没加全」。
- `pnpm migrate:dry-run` 与 `typecheck` / `test` 读的都是**工作树**，三个文件
  在工作树里齐全，本地一路全绿。

后果不对称：漏注册项 = 生产不建列 = **每次咨询提交落库都失败**。
所以这类改动 `git add` 之后必须 `git status` 再扫一眼——宪章禁用
`git add -A` 是对的，但代价就是漏文件由人来兜。
