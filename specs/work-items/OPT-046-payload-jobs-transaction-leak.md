...
### 复现：第二次切流量（`sbh-111` → `sbh-112`，17:21）

silent 修复上线本身又是一次切流量，等于把上面那个实验**独立重做了一遍**：

| | 第一次（`sbh-110`→`111`） | 第二次（`sbh-111`→`112`） |
|---|---|---|
| 争抢次数 | 23（14:36–14:54） | **24**（17:25–17:43） |
| `idle in transaction` | 0 | **0** |
| 存活实例的连接池 | 2 条全 `idle` | 2 条全 `idle` |

两次独立观测、同一结论。这比单次「没看到问题」强得多——单次可以是运气，
两次同量级的争抢都零泄漏，才谈得上是 patch 在起作用。

### 关于 silent 改动本身：生产侧只能证伪，不能证实

必须说清楚这个边界，别把它记成「已在生产验证」：

- `Running N jobs.` 那条 info 日志**只在真有 job 时才打**（`runJobs/index.js` 在无 job 时
  提前 return）。稳态下它本来就是 0——所以「日志里查不到 info」两种情况下都成立，
  **不构成 `info: true` 生效的证据**。
- 「error 现在放行了」需要真有任务抛错才看得见，而不该为了验证去生产造数据。

所以这个改动的证据是**单测（双向验过）+ Payload 源码判据**；生产侧只确认了
「没有出现 info 噪音刷屏」这个**否定**结论。真正的正面验证要等下一次自然发生的
任务失败——而那正是这次改动的目的：到那时它会被看见。
