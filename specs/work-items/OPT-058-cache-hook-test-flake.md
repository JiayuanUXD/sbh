# Task Packet：OPT-058 `supply-public-cache-hook` 在满负载下 5 秒超时，五次中两次

> 状态：**已确认，未修复**
> 创建日期：2026-08-26
> 来源：OPT-053 实施期间反复撞到，一度拦下 `git push`（pre-push 跑全套单测）
> 编号说明：本项**让号两次**——原编 OPT-056，与后台体验优化批次（PR #111）撞号改取 057；
> 057 又被「移除出售频道开关」（PR #112）占用，最终定为 058。教训是开工前查
> `specs/work-items/` 的现有编号，别只看自己分支上有没有。
