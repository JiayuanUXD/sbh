...
- 进行中：Task 1-5 已完成。契约 5 tests + provider/缓存 13 tests PASS（合计全量 2244 PASS）、typecheck PASS、lint PASS（0 error）。
  ...
  注：全量套件仍有 3 个 pre-existing 失败（非 Task 6 引入，留待 Task 7 处理）——detail-components-contract `<video>`（Task 4 分类 Tab 默认仅渲染图片 Tab，视频未进首屏 SSR）、frontend-mappers latitude/longitude x2（Task 3 mapBuildingDetail DTO 暴露 coordinates 与 P0 隐私契约冲突）。Task 6 自身 29 测试通过，未引入新回归。
