...
**实施时必须先实测的两个未知**（本设计未验证，别当结论）：

- Payload 3.86 里 field 级 `admin.hidden` 是否影响 form state 与 `required` 校验的报错呈现。
  注意这与 Global 的 `admin.hidden` 不同——后者会连路由一起排除（OPT-053 的教训）。
- `useForm().dispatchFields` 跨字段写入后，`required` 字段的校验状态是否正确刷新。

若任一为真且无解，退路是**形态改为「联动下拉」**：保留 N 个原生下拉，只做按父级收窄，
不做单框级联。§4 的校验层与 §5 的数据层不受影响。
