...
5. 本条为历史原始门：当前 MP-106/107 代码已完成；在 MP-105 验收门通过前，MP-106/107 不得进入真实集成验收或合并发布，避免用后续功能掩盖当前闭环的环境缺口。
...
在上述环境门未齐备前，允许把“代码传输层、预检工具与 Node 验证完成”提交到功能分支，但状态必须保持“待 AppID/staging 关联、开发者工具、图片/COS、隐私与真机验收”。当前 MP-106/107 代码已完成，但 MP-106/107 不得进入真实集成验收或合并发布；只读的需求讨论、设计与合同草案不受此限制。
