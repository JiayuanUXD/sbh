...
> 状态：**已上线**（PR #195 → master `d403a9a`，2026-09-16 全量切流；线上验收见 `artifacts/verification/OPT-099/online/`）
> 创建日期：2026-09-16
...
- [x] **上线后用同一份 Playwright 脚本复测生产站**：24 张真实封面（原始比例 7 种）→ 媒体盒比例 **8 种 → 1 种**、卡高 **两档 → 315.8**；首页供给卡 3:2 → 16:10；商圈级联在 2181 套真实数据上走通（黄浦 → 淮海中路 → 切静安清商圈）
