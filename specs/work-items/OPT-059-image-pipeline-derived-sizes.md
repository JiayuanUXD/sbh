...
**结果**：用户在本机浏览器人工走查后确认**通过**，本项关闭。

**口径**：这三条视觉结论是**人工看过判定的，不是 agent 实测的**——本次会话的
Browser pane 始终不合成帧，三张 PNG 一张也没产出。别把它当成有截图证据的结论。

关闭前另有一轮真实 DOM 验证（agent 实测，非人工）覆盖了机制层，且**这些是最终
审查那轮修复后才有的行为，此前从未验过**：类型卡确实发出了三档 `srcset` 与
三段式 `sizes`；房源列表卡是 `768w` 换值且 `srcset` 为空（2MB 红线守住）；
bento 按坑位分别报 `62vw`/`32vw`；焦点设 20/80 后 `--focal-x/--focal-y` 如实
出现在图上；未换图的存量卡仍无 `srcset`、焦点落 `50%`（零变化契约成立）。
