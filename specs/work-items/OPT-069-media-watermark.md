...
- `usage`：`listing-photo` | `brand` | `article` | `other`。**只有 `listing-photo` 走水印。**
  - **默认值 `listing-photo`**。理由：误打留得下退路（`media-source/` 有干净原件，
    可人工取回覆盖），漏打不可逆（无水印图已经流出）。默认值要偏向留得下退路的那一侧。
  ...
  - 品牌素材（logo、落地页背景）靠两条兜住：存量由 §6.1 的回填脚本纠正，
    ...
    且干净原件还在 `media-source/`，不是不可逆损失。
  - 复原是**人工**的（2026-09-04 整支评审订正，此前这里写的是「改 `usage` + 重刷即可复原」，
    而那条路不存在）：`selectRebakeTargets` 只处理 `usage === 'listing-photo'`，把误标的图
    改成 `brand` 之后重刷根本不会选中它，水印图会原样留着。实际步骤是：先把 `usage` 改对，
    再从 `media-source/<filename>` 取回干净原件、以同名重新上传——`bakeAfterUpload` 见
    `usage` 不是实景图就只落干净字节并清掉 `watermark.version`。**刻意不做「反烘焙」分支**：
    它只会多一条要维护的写字节路径，而这件事的频率低到人工足够。
- `watermark`：只读元数据 `{ version, appliedAt }`，供重刷任务判定幂等。
  - `version` 是**当前水印配置的内容哈希**（对 §5.5 那组参数 + 渲染器版本号 + 字体栈
    + 容器字体包名求哈希；后两项是 2026-09-04 整支评审补进去的，理由见 §7.3），
...
- `src/domain/media/watermark.ts` 的 `WATERMARK_FONT_FAMILY` 把该包注册的字体族名
  ...
  Windows/macOS 开发时的兜底——生产容器里这几项都不存在。
  - **订正（2026-09-04 整支评审 + 容器实测）**：此前这里写「不放在首位就白装」，那是错的。
    在容器等价的 fontconfig 环境下只改这串 family、其余入参全同，两次渲染**逐字节相同**——
    fontconfig 做最佳匹配而非精确匹配失败，系统里只要有覆盖该码点的字体就会被选中。
    当前镜像只装一个 CJK 字体，**栈的顺序不是失效点**；镜像里一旦多出第二个 CJK 字体，
    栈首才会重新成为决定因素。
- `WATERMARK_FONT_PACKAGE`（= Dockerfile 装的那个包名）**进版本哈希**：真正决定像素的是
  装了哪个包。换成 `fonts-noto-cjk` 会让字形全变，而哈希若不动，之后每一轮重刷都判
  「已是当前版本」跳过，新旧两种字形永久共存且没有任何报错——与「改了渲染逻辑却忘了 +1
  版本号」完全同构，正是本功能用内容哈希要摆脱的依赖。
- 守卫测试 `tests/media-watermark-font-guard.test.ts`：断言 Dockerfile 确实装了 CJK
  字体包、装的**包名**等于 `WATERMARK_FONT_PACKAGE`、且族名与 `WATERMARK_FONT_FAMILY`
  首项一致。这是唯一会在「以后有人为镜像瘦身删掉这行」或「换个字体包」时变红的信号；
  换包时它会逼着改常量，哈希随之变化，存量图下一轮重刷自动重烘。
...
这个顺序现在由代码保证，不再只靠手册（2026-09-04 整支评审）：`backfill-media-sizes.ts`
靠「HTTP 读回当前母版 → 同名回写」补派生，而烘过之后那个母版**已经带水印**，回写会被
`watermarkPlugin` 当成一次新上传——把带水印的字节备份进 `media-source/` 当「干净原件」
（干净副本永久丢失），再叠第二层水印。该脚本因此显式跳过 `watermark.version` 有值的记录，
并把跳过张数单独报出来。
