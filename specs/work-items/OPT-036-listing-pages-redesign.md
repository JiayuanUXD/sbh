...
3. **`findEffectiveBuildings` 的 200 条硬上限**：`searchBuildingsFiltered` 继承 supply-adapter 的
   ...
   **本批次让它的后果升级了一档（全批次终审补记，排期时按这个新事实定优先级）**：
   页头现在印「收录 N 个楼盘」，N 取自 `unfilteredTotalDocs`，而它同样来自被截断的
   那 200 条。于是**超过 200 个楼盘的城市，页面上会显示一个用户看得见的错误数字**
   ——一个城市明明收录了 340 个楼盘，页头写「收录 200 个楼盘」。改版前这一页没有
   这个数字，截断只表现为「列表短了一截」（不好，但不构成对外说了假话）；
   现在它变成一句会被引用、会被截图、会被拿去和后台数据对账的具体断言。
   短期缓解可选「触顶时不印这个数」，根治仍是分页适配器。
