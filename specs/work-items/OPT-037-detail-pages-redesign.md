...
- 页面顺序：标题栏 → 核心区（画廊 + 决策卡）+ tab 区 → 描述 → 周边与交通 → 所在楼盘
...
- tab 区通栏 `grid-column 1 / -1`、`grid-row 2`、`margin-top 24`
...
- tab pill `36` 高、`padding 0 18`、选中 `#1d1d1f`/`#fff`、`gap 8`；tab 到面板 `12px`；**默认选中「房源概况」**
- 概况：组间距 `40px`、组标签 `13/600/--ink-3`、行 `min-height 44`、键 `15/400/--ink-2`、值 `15/500/--ink`
- **组区分只用间距 + 组标签，不用顶线不用色块**
- 租金账「这套房一个月要花多少」：
  - 等式块宽 **700px**（通栏面板内左对齐，不随 1180 拉宽）
  - 行网格 `448 / 24 / 132 / 96`（键 / 运算符 / 值 / 单位）
  - 右栏「计算口径」`1fr`（约 328）、`gap 72`、`13/1.55` + 顶 hairline
  - 行高：常规 `44`、合计 `52`、`padding 11px 0`
  - 字号：键 `15/400`、值 `17/500`、合计键 `15/600` 值 `24/600`
  - 合计行上边线 `1px rgba(0,0,0,.16)` + `padding-top 16`；等式组间距 `32`
  - **数据来源已确认存在**：`Listings.paymentTerms` / `depositMonths` / `propertyFeeInclusion` / `propertyFeeAmount`，`Buildings.propertyFee` / `parkingFee`。**缺失项必须显示 `—` 并在计算口径里说明该项未计入，不得静默按 0 参与合计**
...
- 状态走查：无图房源、无坐标楼盘、租金账缺字段、供给区三组各自为空、超长标题
