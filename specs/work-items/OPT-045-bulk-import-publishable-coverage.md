...
## 10.5 实施期踩到的两个坑（同一个改动引出的）

### 10.5.1 模板加列会让旧表格被整份拒收

`parseWorkbook` 对期望列做「一个都不能少」的校验。直接往 `XXX_COLUMNS` 加列，
运营手上所有已有表格下次导入会被 `MISSING_COLUMNS` 拒收——**而那些表格完全合法**。

当时代码注释、提交信息、单测里都写着「旧表格原样继续可用」，**那是没验证过的断言**，
靠 CI 的 `bulk-import.spec`（夹具就是旧格式表）才发现。

### 10.5.2 修 10.5.1 时造出了更隐蔽的第二个坑

把 `expectedColumns` 从完整列改成必需列——但那个参数**同时**被用在两处：

```ts
const missing = expectedColumns.filter(...)      // ① 存在性校验 ← 想改的
for (const column of expectedColumns) { ... }    // ② 行映射   ← 跟着变了
```

于是新增的 11 列**全部被静默丢弃**。生产验收实测：楼盘的等级/竣工/在售单价
落库全 null，两条出售房源判 `PRICE_REQUIRED`（「售价」读到空串），
批次记录的 `rawRows` 里只有 9 个原始键。

**而批次状态是 `completed`、`0 失败`**——值凭空消失，没有任何信号。

修法是拆成两个参数（`requiredColumns` / `readColumns`），职责彻底分开。

### 10.5.3 为什么测试没抓到——这条比缺陷本身更值得记

修 10.5.1 时加了三条测试：「必需列恰好是原八列」「必需列是模板列的前缀」
「新列不在必需列里」。**三条全过，一条都没发现 10.5.2。**

因为它们断言的是**列表的内容**，而缺陷在于**这个列表被用在了哪里**。
e2e 夹具又是旧格式，同样覆盖不到新列。

新测试 `tests/supply-import-workbook-columns.test.ts` 改走**真实 exceljs 工作簿**，
验行为而不是列表，并且**双向验证过**（临时改回缺陷版，6 条里立刻红 3 条）。

> 通用教训：为「防止 X 再发生」加的测试，要验的是 **X 的症状**，不是
> **导致 X 的那个数据结构长什么样**。后者写起来容易、跑得快、看着很像覆盖了，
> 但换一种方式犯同类错误时它照样全绿。
